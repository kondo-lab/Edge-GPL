...
/*
	Name: add_edge
	Function: Sends a request to add a new edge along with relevant data
		to the server. Receives the operation status from server.
	Input: Names of the source and target vertices, edge weight.
*/
namespace Client {
STATUS add_edge(socket_info *arg, string name_from, string name_to, int weight)
{
	...
	while (true) {
		...
		send(socket_operation, &weight, sizeof(weight), 0);
		PRINT_UI("Sent data.");
		...
		if (recv(socket_operation, &status,
			...
			PRINT_STATUS("Edge added.");
		} else {
			PRINT_STATUS("Failed to add edge.")
		}
