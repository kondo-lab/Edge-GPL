...
/*
	Name: add_vertex
	Function: Sends a request to add a new vertex along with relevant data
		to the server. Receives the operation status from server.
	Input: Names of the vertex to be added.
*/
namespace Client {
STATUS add_vertex(socket_info *arg, string name)
{
	...
	STATUS status = SUCCESS, // Marks whether the requested operation was successful or not
	       ack = FAIL;
	COMMAND command = CMD_ADD_VERTEX;
	...
	while (true) {
		...
		if (recv(socket_operation, &status,
			...
			PRINT_STATUS("Vertex added.");
		} else {
			PRINT_STATUS("Failed to add vertex.")
		}
