...
/*
	...
	Function: Sends a request to change the current graph format along
		with relevant data to the server. Receives the operation
		status from server.
	Input: Format to change into.
*/
namespace Client {
STATUS change_graph_format(socket_info *arg, int format)
{
	...
	STATUS status = SUCCESS, // Marks whether the requested operation was successful or not
	       ack = FAIL;
	COMMAND command = CMD_CHANGE_GRAPH_FORMAT;
	...
	while (true) {
		...
		send(socket_operation, &format, sizeof(format), 0);
		PRINT_UI("Sent data.");
		...
		if (recv(socket_operation, &status,
			...
			PRINT_STATUS("Changed graph format.");
		}
		else {
			PRINT_STATUS("Failed to change graph format.");
		}
