...
/*
	Name: clear_graph
	Function: Sends a request to clear the current graph to the server.
...
STATUS clear_graph(socket_info *arg)
{
	...
	int operation_port = socket_information->operation_port;
	...
	STATUS status = SUCCESS, // Marks whether the requested operation was successful
	       ack = FAIL;
	COMMAND command = CMD_CLEAR_ALL_GRAPH;
	...
	while (true) {
		...
		PRINT_UI("Sending data... to " << socket_information->host_backup);
		...
		if (recv(socket_operation, &status,
			...
			PRINT_STATUS("Graphs cleared.");
		} else {
			PRINT_STATUS("Failed to clear graphs.")
		}
