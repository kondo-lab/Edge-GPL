...
/*
    Name: export_graph
    Function: Sends a request to export the current graph along with
        relevant data to the server. Receives the operation status
        from server.
    Input: Name of the file to hold the exported graph.
*/
namespace Client {
STATUS export_graph(socket_info *arg, string filename, bool is_encryption)
{
    ...
    STATUS status = SUCCESS, // Marks whether the requested operation was successful
           ack = FAIL;
    COMMAND command = CMD_EXPORT_GRAPH;
    ...
    while (true) {
        ...
        if (recv(socket_operation, &status,
            ...
            PRINT_STATUS("Exported graph.");
        } else {
            PRINT_STATUS("Failed to export graph.")
        }
