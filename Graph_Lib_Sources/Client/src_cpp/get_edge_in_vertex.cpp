...
/*
	...
	Function: Sends a request to retrieve all in-edges of a vertex
		along with relevant data to the server.
		Receives the operation status and the list of in-edges
		from server.
	Input: Name of the vertex whose in-edges are to be retrieved.
*/
namespace Client {
STATUS get_edge_in_vertex(socket_info *arg, string name,
		vector<string> *sources, vector<int> *weights)
{
	...
	STATUS status = SUCCESS, // Marks whether the requested operation was successful or not
	       ack = FAIL;
	COMMAND command = CMD_EDGE_IN_VERTEX;
	...
	while (true) {
		...
		if (recv(socket_operation, &status,
			...
			for (int i = 0; i < datasize; i++) {
				string source = "";
				int weight = 0;
				if (socket_recv(socket_operation, source) == FAIL) {
					status_data = FAIL;
				}
					...
					PRINT_STATUS("Source vertex: " + source
						+ " -- "
