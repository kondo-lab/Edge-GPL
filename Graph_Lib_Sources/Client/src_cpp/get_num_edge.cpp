...
/*
	Name: get_num_edge
	Function: Sends a request to retrieve the number of edges in the
		current graph.
		Receives the number of edges from server.
	Input: none
*/
namespace Client {
STATUS get_num_edge(socket_info *arg, int *num_edge)
{
	...
	while (true) {
		...
		if (recv(socket_operation, &data,
			...
			PRINT_STATUS("The graph has " + to_string(data) + " edges.");
		}
                if (num_edge != NULL)
                        *num_edge = data;
