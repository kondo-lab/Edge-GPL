...
/*
	Name: get_num_vertex
	Function: Sends a request to retrieve the number of vertices in the
		current graph.
		Receives the number of vertices from server.
	Input: none
*/
namespace Client {
STATUS get_num_vertex(socket_info *arg, int *num_vertex)
{
	...
	STATUS status = SUCCESS, // Marks whether the requested operation was successful
	       ack = FAIL;
	COMMAND command = CMD_NUM_VERTEX;
	int data = 0;
	...
	while (true) {
		...
		if (recv(socket_operation, &data,
				sizeof(data), 0) <= SUCCESS) {
			PRINT_STATUS("Failed to receive processed data.");
			status = FAIL;
		}
		else {
			PRINT_STATUS("The graph has " + to_string(data) + " vertices.");
		}
		if (num_vertex != NULL)
			*num_vertex = data;
