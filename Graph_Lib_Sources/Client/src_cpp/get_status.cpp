...
/*
	Name: get_status
	Function: Sends a request to retrieve the last sent command to the server.
		Receives the latest command prior to the current command.
	Input: none
*/
namespace Client {
STATUS get_status(socket_info *arg, int *cmd)
{
	...
	while (true) {
		...
		if (recv(socket_operation, &latest_command,
				sizeof(latest_command), 0) <= SUCCESS) {
			PRINT_STATUS("Failed to receive processed data.");
			status = FAIL;
		}
		else {
			PRINT_STATUS(print_command(latest_command));
		}

		// 8. Close operation socket
