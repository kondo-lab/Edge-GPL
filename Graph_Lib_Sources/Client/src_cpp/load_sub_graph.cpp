...
STATUS load_sub_graph(socket_info *arg, string filename, string &str_graph, bool is_encryption)
{
    ...
    STATUS status = SUCCESS, // Marks whether the requested operation was successful or not
           ack = FAIL;
    COMMAND command = CMD_LOAD_SUB_GRAPH;
    ...
    while (true) {
        ...
        if (recv(socket_operation, &status, sizeof(status), 0) <= SUCCESS) {
            PRINT_STATUS("Failed to receive processed data.");
            status = FAIL;
        }
        else {
            STATUS ack = SUCCESS;
            send(socket_operation, &ack, sizeof(ack), 0);

            if (socket_recv(socket_operation, str_graph) == FAIL) {
                PRINT_STATUS("Failed to receive processed data.");
                status = FAIL;
            }
            PRINT_UI("Subgraph: " << str_graph);
        }
