...
/*
	Name: print_graph
	Function: Sends a request to retrieve the current graph to the server.
		Receives the operation status and curernt graph from server.
	Input: none
*/
namespace Client {
STATUS print_graph(socket_info *arg, string *graph, bool is_print_graph)
{
	...
	STATUS status = SUCCESS, // Marks whether the requested operation was successful or not
	       ack = FAIL;
	COMMAND command = CMD_PRINT_GRAPH;
