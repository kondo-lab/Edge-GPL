...
/*
	Name: remove_edge
	Function: Sends a request to remove an edge along with relevant data
...
STATUS remove_edge(socket_info *arg, string name_from, string name_to)
{
	...
	STATUS status = SUCCESS, // Marks whether the requested operation was successful or not
	       ack = FAIL;
	COMMAND command = CMD_REMOVE_EDGE;
	...
	while (true) {
		...
		if (recv(socket_operation, &status,
			...
			PRINT_STATUS("Edge removed.");
		} else {
			PRINT_STATUS("Failed to remove edge.")
		}
