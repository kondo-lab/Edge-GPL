...
/*
	Name: remove_path
	Function: Sends a request to remove the local shortest path registered before.
*/
namespace Client {
	STATUS remove_path(socket_info* arg)
	{
		...
		STATUS status = SUCCESS, // Marks whether the requested operation was successful or not
			   ack = FAIL;
		COMMAND command = CMD_REMOVE_PATH;
		...
		while (true) {
			...
			if (recv(socket_operation, &status,
				...
				PRINT_STATUS("Path removed.");
			}
			else {
				PRINT_STATUS("Failed to remove path.")
			}
