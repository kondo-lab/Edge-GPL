...
/*
	Name: remove_vertex
	Function: Sends a request to remove a vertex along with relevant data
		to the server. Receives the operation status from server.
	Input: Name of the vertex to be removed.
*/
namespace Client {
STATUS remove_vertex(socket_info *arg, string name)
{
	...
	STATUS status = SUCCESS, // Marks whether the requested operation was successful or not
	       ack = FAIL;
	COMMAND command = CMD_REMOVE_VERTEX;
	...
	while (true) {
		...
		if (recv(socket_operation, &status,
			...
			PRINT_STATUS("Vertex removed.");
		} else {
			PRINT_STATUS("Failed to remove vertex.")
		}
