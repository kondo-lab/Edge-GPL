...
/*
	...
	Function: Sends a request to run data concatenation algorithm on a
		side-server along with relevant data to the server.
		Receives the operation status and number of concatenated edges
		from server.
	Input: Name of the root vertex and side-server IP address.
*/
namespace Client {
STATUS run_data_concat(socket_info *arg, string name)
{
	...
	while (true) {
		...
		if (recv(socket_operation, &status,
			...
			if (recv(socket_operation, &added_edges,
					sizeof(added_edges), 0) <= SUCCESS) {
				PRINT_STATUS("Failed to receive processed data.");
				status = FAIL;
			}
			else {
				PRINT_STATUS("Added " + to_string(added_edges) + " edges.");
			}
		}
		else {
			PRINT_STATUS("Failed to perform requested operation.");
		}
