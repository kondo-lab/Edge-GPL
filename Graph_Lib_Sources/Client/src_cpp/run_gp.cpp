...
/*
	Name: run_gp
	Function: Sends a request to run graph partitioning algorithm along with relevant data
		to the server.
		Receives the operation status from server.
	Input: Number of parts and graph-cut type.
*/
namespace Client {
	STATUS run_graph_partition(socket_info* arg, int num_parts, int cut_type)
	{
		...
		STATUS status = SUCCESS, // Marks whether the requested operation was successful or not
			   ack = FAIL;
		COMMAND command = CMD_GRAPH_PARTITION;
		...
		while (true) {
			...
			send(socket_operation, &num_parts, sizeof(num_parts), 0);
			send(socket_operation, &cut_type, sizeof(cut_type), 0);
			PRINT_UI("Sent data.");
			...
			if (recv(socket_operation, &status,
				...
				PRINT_STATUS("Graph partitioning finished.");
			}
			else {
				PRINT_STATUS("Failed to run graph partitioning.")
			}
