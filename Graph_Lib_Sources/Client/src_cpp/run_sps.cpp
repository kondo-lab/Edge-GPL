...
/*
	Name: run_sps
	Function: Sends a request to run SPS algorithm along with relevant data
		to the server.
		Receives the operation status and list of vertices traversed by SPS
		from server.
	Input: Names of the SPS source and target vertices.
*/
namespace Client {
STATUS run_sps(socket_info *arg, string name_from, string name_to, string *list)
{
	...
	while (true) {
		...
		socket_send(socket_operation, name_from);
		socket_send(socket_operation, name_to);
		PRINT_UI("Sent data.");
...
STATUS run_sps_all_path(socket_info *arg, string name_from, string name_to, string *list)
{
	// 1. Extract socket information
	socket_info *socket_information = (socket_info*) arg;
	string host = socket_information->host;
	...
	STATUS status = SUCCESS, // Marks whether the requested operation was successful or not
	       ack = FAIL;
	COMMAND command = CMD_SPS_ALL_PATH;
	...
	while (true) {
		...
		if (recv(socket_operation, &status,
			...
			if (status_data == SUCCESS) {
				if (dataval == "")
					PRINT_STATUS("There is no eligible path from " + name_from + " to " + name_to + ".")
				else
					PRINT_STATUS(dataval)
				if (list != NULL)
					*list = dataval;
